 ...
 * ========================================== */
/* ==========================================
 * is_safe_district_name - validare stricta a numelui districtului
 * REFUZAM nume care contin: '/', '..', '\0', sau care sunt vide.
 * Fara aceasta verificare, un atacator ar putea face --remove_district "/"
 * si ar sterge tot sistemul de fisiere!
 * ========================================== */
int is_safe_district_name(const char *name) {
    if (name == NULL || name[0] == '\0') return 0;
    if (name[0] == '/') return 0;          /* path absolut - refuzat */
    if (name[0] == '-') return 0;          /* poate fi interpretat ca optiune */
    if (strstr(name, "..") != NULL) return 0;  /* path traversal - refuzat */
    if (strchr(name, '/') != NULL) return 0;   /* nume cu slash - refuzat */
    if (strlen(name) > 63) return 0;       /* prea lung */
    return 1;
}

/* ==========================================
 * cmd_remove_district - sterge intreg directorul districtului
 * MANAGER ONLY. Foloseste fork() + execlp("rm", "-rf", path).
 * Sterge si symlink-ul active_reports-<district>.
 *
 * Validare stricta a numelui districtului inainte de rm -rf
 * pentru a preveni accidente catastrofale.
 * ========================================== */
int cmd_remove_district(Args *args) {
    /* 1. Verificare rol */
    if (args->role != ROLE_MANAGER) {
    ...
    /* 2. VALIDARE STRICTA a numelui (foarte important pentru rm -rf!) */
    if (!is_safe_district_name(args->district)) {
        fprintf(stderr, "Eroare: nume district invalid sau periculos: '%s'\n",
                args->district);
        return -1;
    }

    /* 3. Verifica daca directorul exista */
    struct stat st;
    if (stat(args->district, &st) < 0) {
    ...
    printf("Stergere district '%s'...\n", args->district);

    /* 4. fork() ca sa cream un proces copil */
    pid_t pid = fork();
    if (pid < 0) {
        perror("fork");
        return -1;
    ...
    if (pid == 0) {
        /* COPIL: executa rm -rf <district> */
        execlp("rm", "rm", "-rf", args->district, (char *)NULL);
        /* Daca execlp returneaza, a esuat */
        perror("execlp rm");
        _exit(EXIT_FAILURE);
    }

    /* PARINTE: asteapta copilul sa termine */
    int status;
    ...
    if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
        fprintf(stderr, "Eroare: rm -rf a esuat (status=%d)\n",
                WEXITSTATUS(status));
        return -1;
    }

    printf("Director '%s' sters.\n", args->district);

    /* 5. Sterge si symlink-ul active_reports-<district> */
    char link_name[256];
    snprintf(link_name, sizeof(link_name), "active_reports-%s", args->district);
    if (unlink(link_name) == 0) {
        printf("Symlink '%s' sters.\n", link_name);
    } else {
        if (errno != ENOENT) {  /* daca nu e "doesn't exist", e eroare reala */
            perror("unlink symlink");
        }
    }

    /* 6. Logare - dar atentie: logged_district NU mai exista (l-am sters!),
     * deci scrieam intr-un log central daca am avea unul.
     * Pentru moment, doar afisam pe stdout. */
...
int main(int argc, char *argv[]) {
    ...
    switch (args.command) {
        ...
        case CMD_REMOVE_DISTRICT:   rc = cmd_remove_district(&args); break;
